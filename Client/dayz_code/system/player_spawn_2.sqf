...
while {true} do {
	...
	*/
	...
	//Hunger
	if (_refObj == player) then {
	_hunger = +((((r_player_bloodTotal - r_player_blood) / r_player_bloodTotal) * 5) + (_speed / 2) + dayz_myLoad) * 8;
    } else {
	_hunger = +((((r_player_bloodTotal - r_player_blood) / r_player_bloodTotal) * 5) + dayz_myLoad) * 9;
    };
